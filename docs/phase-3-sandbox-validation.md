...
### Fifth test (skipped in normal mode)

A **real** status write, backup creation, and `legacy:restore` drill is **skipped** unless you opt in (see below).
...
When `SANDBOX_VALIDATE_REAL=1`, the band runs one additional test that:

1. Uses another **fresh** `mkdtemp` sandbox (same synthetic fixtures + marker).
2. Sets `WRITE_MODE=enabled`, `ALLOW_LEGACY_WRITES=I_UNDERSTAND_THIS_IS_A_DISPOSABLE_COPY`, and a temp `BACKUP_DIR`.
3. **Commits** a single `SCHEDULE.STATUS` change for appointment `1001` (synthetic id).
4. Verifies status changed on disk, a backup folder exists under temp `BACKUP_DIR`, then runs **`runLegacyRestore`** from that backup.
5. Verifies `STATUS` reverts to the value before the write.
...
| --- | --- | --- |
| `SANDBOX_VALIDATE_REAL` | unset or not `1` | **`1`** (enables real-write sub-suite) |
...
5. `SANDBOX_VALIDATE_REAL=1 pnpm sandbox:validate`.
...
Vitest reports **4 passed**, **1 skipped** (the real-write band).
...
**5 passed**, **0 skipped** when `SANDBOX_VALIDATE_REAL=1`.
...
| Real band fails when enabled | Backup, restore, or status commit path broken on synthetic tree |
...
| `pnpm sandbox:validate` | Focused signal for sandbox write dry-run + optional synthetic restore drill |
