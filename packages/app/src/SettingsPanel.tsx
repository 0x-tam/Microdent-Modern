...
export function SettingsPanel({
  ...
  const mirrorBanner = resolveMirrorConnectionBanner(bridgePhase, mirrorStatus);
  const mirrorStale =
  ...
  return (
    <div className="app-settings" aria-labelledby="settings-panel-title">
      ...
      <Card className="app-settings__card">
        ...
        <CardBody>
          {writeChip ? (
            <Badge variant={writeChip.variant} className="app-settings__chip" semanticLabel={`Bridge write mode: ${writeChip.label}`}>
              {writeChip.label}
            </Badge>
          ) : (
          ...
          {mirrorBanner ? (
            <p className={`app-settings__mirror-banner app-settings__mirror-banner--${mirrorBanner.tone}`}>
              <strong>{mirrorBanner.label}</strong> — {mirrorBanner.body}
            </p>
          ) : null}
        </CardBody>
      </Card>
          ...
          {bridgePhase !== "connected" ? (
            ...
            <>
              <ul className="app-settings__facts">
                <li>
                  {mirrorStatus.sqliteConfigured
                    ? SETTINGS_MIRROR_SQLITE_CONFIGURED
                    : SETTINGS_MIRROR_SQLITE_MISSING}
                </li>
                <li>
                  {mirrorStatus.sqliteUsable ? SETTINGS_MIRROR_USABLE : SETTINGS_MIRROR_FALLBACK}
                </li>
                <li>
              ...
              {mirrorRefreshError ? (
                <p className="app-settings__error" role="alert">
                  Could not refresh mirror status. Try again.
                </p>
              ) : null}
              {runs.length === 0 ? (
                <p className="app-settings__muted">
                  {SETTINGS_MIRROR_NO_RUNS} {SETTINGS_MIRROR_NO_RUNS_HINT}{" "}
                  <span className="app-settings__doc-ref">{SETTINGS_MIRROR_DOC_LINK}</span> (
                  <code>docs/phase-4-mirror-import-operator.md</code>)
                </p>
              ) : (
